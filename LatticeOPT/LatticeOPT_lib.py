        ...
        for i in range(s[0] - rb):
            for j in range(s[1] - cb):
                if (threshold == None) or ((threshold != None) and (cArry[eid] > threshold)):
                    edge_pts = EdgeConnectivity(Type, i, j)
                    if ExcSet[eid]:
                        plt.plot([cx[edge_pts[0, 1]], cx[edge_pts[1, 1]]], [cy[edge_pts[0, 0]], cy[edge_pts[1, 0]]],
                                 'k')
                    else:
                ...
                if idx == 0:
                    ...
                    for l in range(N_ele_per_edge):
                        ...
                        for k in range(Nz):
                            ...
                            my_edge_thickness[my_element_id] = GetShellThickness(thickness_dist, edge_id,
                                                                                 my_normalized_z_coord)  # Store element thickness
    ...
    for i in range(len(Lines)):
        ...
        if ('*End Part' in l) and not written:
            ...
            for edge_id, edge_set in Edge_sets.items():
                ...
                f.write(
                    '*Distribution, name=' + dist_name + ', location=ELEMENT, Table=' + dist_name + '_Table\n,     1.\n')
                ...
                f.write('*Shell Section, elset=Edge-' + str(
                    edge_id) + ', material=MAT, controls=EC-1, shell thickness=' + dist_name + '\n1., 5\n')
...
def CheckStatus(jn, itr):
    try:
        ...
        if 'COMPLETED' not in L[-1]:
            print('                                                         WARNING: Itr ' + str(
                itr) + ' failed, continue w/ existing data')
        else:
            print(
                '                                                         Itr ' + str(itr) + ' completed successfully')
            # Clean up
            List_delete = ['.prt', '.sim', '.sta', '.abq', '.sel', '.mdl', '.stt', '.res', '.dat', '.pac', '.msg',
                           '.com', '.log']
...
def ComputePeriodicMtx(Nx, Ny, N_cell_x, N_cell_y, ExcSet):
    ...
    for row in range(Ny):
        for col in range(Nx + 1):
            ...
            forward_mask[
                offset + row * (Nx + 1) + col] = reduced_offset + reduced_row * free_x_edges_per_cell + reduced_col
...
def UpdateThickness(edge_energy, t_old, thickness_weight, wl, m_new, t_max, t_trheshold, max_dt, Objective_type,
                    max_inner_solver_iter, itr):
    ...
    res = minimize(OBJ, t_old, method='trust-constr', jac=OBJ_jac, hess=OBJ_hess,
                   constraints=cons, options={'gtol': 1e-5, 'disp': True, 'maxiter': max_inner_solver_iter},
                   bounds=bnds, callback=OBJ_watch_dog)
...
def DesignUpdate(itr, t_max, t_trheshold, max_dt, Objective_type):
    ...
    curr_sum_t = np.sum(np.multiply(t_old[IncSet], wall_length[
        IncSet]))  # When calculating current sum of thickness, only include the edges in the optimization
    ...
    print('\nSanity check:\nMin wall thickness = ', '{:1.3f}'.format(np.min(thickness_new[IncSet])),
          ', Max wall thickness = ', '{:1.3f}'.format(np.max(thickness_new[IncSet])))
    print('Max thickness change = ', '{:1.4f}'.format(observed_max), '\nActual mass / expected mass = ',
          '{:1.2f}'.format(ratio * 100.))
